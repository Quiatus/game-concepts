...
export const calculateHappiness = (isNewMonth, pop, gold) => {
    ...
    if (!calculatedHappiness && gameData.basicResources.pop > 10) {
        ...
        saveGame(gameData)
        if (isNewMonth) actionRiot(pop, gold)
...
const actionRiot = (pop, gold) => {
    ...
    printMessage(`Our people are rioting! <span class='text-bold'>${converThousand(deadPop)}</span><img class='img-s' src='media/pop.png'>died during riots. Rioters broke into our storages and stole <span class='text-bold'>${converThousand(stolenGold)}</span><img class='img-s' src='media/gold.png'>.`, 'critical')
